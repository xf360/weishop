<template>
  <div>
    ...
    <detail-list layout="grid" :col="1">
      <detail-list-item term="上级代理">{{info.invitUserName}}</detail-list-item>
    ...
    <detail-list title="代理详情" layout="grid" :col="2">
      ...
      <detail-list-item term="头像">
        <img height="50" width="50" :src="$http.api+'api/AbpFile/Show?id='+info.file.id" @click="showbig(info.file.id)"/>
      </detail-list-item>
    </detail-list>
    <a-divider style="margin-bottom: 32px" />
    <detail-list title="转账信息" layout="grid" :col="2">
      ...
      <detail-list-item term="打款账户">{{info.payAcount}}</detail-list-item>
    ...
    <a-modal destroyOnClose :maskClosable="false" title="封号" v-model="lockshow" @ok="handleLockOk" cancelText="取消" okText="确认">
