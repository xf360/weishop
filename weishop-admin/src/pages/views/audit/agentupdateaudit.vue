...
<script>
  ...
  import agengtinfo from './agentupdateauditinfo.vue'
  export default {
    components: {
      auditresult,
      agengtinfo
    ...
    data() {
      return {
        ...
        columns: [{
          ...
          title: '原等级',
          dataIndex: 'oldAgencyLevelName'
        },{
          title: '升级等级',
          dataIndex: 'toAgencyLevelName'
        }, {
          title: '联系电话',
          dataIndex: 'tel'
        }, {
    ...
    methods: {
      ...
      async loadstatic() {
        var ret = await this.$http.Get('/api/services/app/B_AgencyUpgrade/GetCount');
        if (ret.success) {
      ...
      async loadlist() {
        this.loading = true
        var ret = await this.$http.Get('/api/services/app/B_AgencyUpgrade/GetList', this.params);
        this.loading = false
        if (ret.success) {
      ...
      async auditpass() {
        var ret = await this.$http.Post('/api/services/app/B_AgencyUpgrade/Audit', {
          id: this.selectid,
          isPass: true
        })
        if (ret.success) {
      ...
      async handleReasonOk() { //审核不通过并提交原因
        this.$refs.reasoncom.form.validateFields(async (err, values) => {
          if (!err) {
            ...
            var ret = await this.$http.Post('/api/services/app/B_AgencyUpgrade/Audit', values);
            if (ret.success) {
