  ...
  <div>
    ...
    <a-modal :maskClosable="false" destroyOnClose title="个人中心" @ok="saveheader" v-model="infovisible" cancelText="取消"
      ...
      <a-row >
        ...
        <a-col :span="16">
          <a-upload name="avatar" listType="picture-card" class="avatar-uploader" :showUploadList="false"
            :action="uploadapi" @change="filechange">
            <img v-if="myinfo.file.id" :src="avatar" alt="头像" />
            <div v-else>
      ...
      <a-row>
        ...
        <a-col :span="16">{{myinfo.userName}}</a-col>
      </a-row>
      <a-row>
        <a-col :span="8" style="text-align:right">手机号：</a-col>
        <a-col :span="16">{{myinfo.phoneNumber}}</a-col>
      </a-row>
      <a-row>
  ...
  export default {
    ...
    data() {
      return {
        ...
        uploadapi: this.$http.api + 'api/AbpFile/Post',
        myinfo: {},
        infovisible: false,
        newpwd: {},
        pwdvisible: false,
    ...
    methods: {
      ...
      async getself() {
        ...
        if (ret.success) {
          ...
          this.avatar = this.$http.api + 'api/AbpFile/Show?id' + this.myinfo.file.id;
        }
      },
      savepwd() {
