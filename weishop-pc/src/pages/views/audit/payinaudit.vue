...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async loadstatic() {
        var ret = await this.$http.Get('/api/services/app/B_PaymentPrepay/GetCount');
        if (ret.success) {
      ...
      async loadlist() {
        this.loading = true
        var ret = await this.$http.Get('/api/services/app/B_PaymentPrepay/GetList', this.params);
        this.loading = false
        if (ret.success) {
      ...
      onChange() {},
      async auditpass() {
        var ret = await this.$http.Post('/api/services/app/B_PaymentPrepay/Audit', {
          id: this.selectid,
          isPass: true
        })
        if (ret.success) {
